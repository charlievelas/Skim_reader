...
// Function to generate combinations of any size (from 1 to vec.size()) 
// I believe that this function is making a duplicate of the final entry, resolution not urgently requried
void generate_combinations(const std::vector<std::string>& vec, int start, int k, std::vector<std::string>& current, std::vector<std::string>& LVCombos_names, std::vector<std::string>& LVCombos_formulae) {
    // If the current combination size is equal to the desired size (k), convert to string and add to LVCombos_names and LVCombos_formulae
    if (current.size() == k) {
        std::string name_combination;
        std::string formula_combination;

        // Convert current combination to a string for LVCombos_names (with underscores)
        ...
        LVCombos_names.push_back(name_combination);  // Store the combination in LVCombos_names

        // Convert current combination to a string for LVCombos_formulae
        for (int i = 0; i < current.size(); ++i) {
            formula_combination += current[i] + "_LV";
            if (i != current.size() - 1) {
                formula_combination += "!$%&*";  // Separate values with a '!£$%&', this string is chosen so that each character can be replaced in the variable definition section
            }
        }
        LVCombos_formulae.push_back(formula_combination);  // Store the combination in LVCombos_formulae
    ...
    for (int i = start; i < vec.size(); ++i) {
        ...
        generate_combinations(vec, i + 1, k, current, LVCombos_names, LVCombos_formulae);  // Recurse for the next element
        current.pop_back();  // Backtrack
    }
}

// Function to generate all combinations of sizes from 2 to vec.size() for both LVCombos_names and LVCombos_formulae
void generate_all_combinations(const std::vector<std::string>& vec, std::vector<std::string>& LVCombos_names, std::vector<std::string>& LVCombos_formulae) {
    std::vector<std::string> current;
    // Generate combinations for each size from 2 to vec.size()
    for (int k = 1; k <= vec.size(); ++k) {
        generate_combinations(vec, 0, k, current, LVCombos_names, LVCombos_formulae);  // Generate combinations of size k
    }
}
...
for (int i = 0; i < LVCombos_names.size(); ++i) {
    if (i>1*PID_vect.size()){
        std::string InvCombo = LVCombos_formulae[i];
        std::replace(InvCombo.begin(), InvCombo.end(), '!', ' ');
        std::replace(InvCombo.begin(), InvCombo.end(), '$', ' ');
        std::replace(InvCombo.begin(), InvCombo.end(), '%', '+');
        std::replace(InvCombo.begin(), InvCombo.end(), '&', ' ');
        std::replace(InvCombo.begin(), InvCombo.end(), '*', ' ');
        outFile << "        InvMass_" + LVCombos_names[i]  + " = (" + InvCombo + ").M();" << endl;
    }
    std::string MissCombo;
    if (i==LVCombos_names.size()-1){
        MissCombo = LVCombos_formulae[i];
    } else{
        MissCombo = LVCombos_formulae[LVCombos_names.size()-i-2];
    }
    std::replace(MissCombo.begin(), MissCombo.end(), '!', ' ');
    std::replace(MissCombo.begin(), MissCombo.end(), '$', ' ');
    std::replace(MissCombo.begin(), MissCombo.end(), '%', '-');
    std::replace(MissCombo.begin(), MissCombo.end(), '&', ' ');
    std::replace(MissCombo.begin(), MissCombo.end(), '*', ' ');
    outFile << "        MissMass_" + LVCombos_names[i]  + " = (" + "Beam_LV  +  Target_LV - " + MissCombo + ").M();" << endl;
    outFile << "        MissMass2_" + LVCombos_names[i]  + " = (" + "Beam_LV  +  Target_LV - " + MissCombo + ").M2();" << endl;
    std::string EnergyCombo;
    if (i==LVCombos_names.size()-1){
        EnergyCombo = LVCombos_formulae[i];
    } else{
        EnergyCombo = LVCombos_formulae[LVCombos_names.size()-i-2];
    }
    std::replace(EnergyCombo.begin(), EnergyCombo.end(), '!', '.');
    std::replace(EnergyCombo.begin(), EnergyCombo.end(), '$', 'E');
    std::replace(EnergyCombo.begin(), EnergyCombo.end(), '%', '(');
    std::replace(EnergyCombo.begin(), EnergyCombo.end(), '&', ')');
    std::replace(EnergyCombo.begin(), EnergyCombo.end(), '*', '-');
    outFile << "        MissEnergy_" + LVCombos_names[i]  + " = " + "Beam_LV.E()  +  Target_LV.E() - " + EnergyCombo + ".E();" << endl;
}
if (evn_el_count==1){
    outFile << "        // Beam-electron(scattered) kinematics" << endl;
...
outFile << endl;
outFile << "    }" << endl;
outFile << "   cout << \"   Number of saved events (additional conditions considered): \" + std::to_string(eventnumber) << endl;" << endl;
outFile << "}" << endl;
