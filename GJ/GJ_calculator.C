...
void GJ_calculator(string filename, string el_input, string pro_input, string p1_input, string p2_input){
...
string filename_new = "GJ_" + filename;
TFile fFileNew(filename_new.c_str(), "RECREATE");
TTree* fTreeNew = fTree->CloneTree(0);

// Add GJ branches to tree
double GJcosTh;
double GJphi;

TBranch* GJcosTh_branch = fTreeNew->Branch("GJcosTh", &GJcosTh, "GJcosTh/D");
TBranch* GJphi_branch = fTreeNew->Branch("GJphi", &GJphi, "GJphi/D");
